...
use App\Http\Controllers\Author\ArticleEditRequestController;
    ...
    public function update(Request $request, Article $article)
    {
        try {
            ...
            if ($article->status === 'published' || $article->status === 'pending') {
                ...
                // Chỉ cho phép cập nhật nếu đang ở trạng thái 'editing' HOẶC
                // nếu bài viết được duyệt chỉnh sửa và chưa chỉnh sửa lần nào sau khi duyệt
                if ($article->status !== 'editing' && !($editStatus['status'] === 'approved' && !$hasBeenEdited)) {
                    return redirect()
