...
class AuthUserController extends Controller
{
    ...
    public function processSignup(Request $request)
    {
        ...
        // Gửi OTP qua email
        Mail::raw("Mã OTP của bạn là: $otp (có hiệu lực trong 60 giây)",
        ...
        return redirect()
            ...
            ->with('status', 'OTP đã được gửi đến email của bạn.');
    }
    ...
    public function verifyOtp(Request $request)
    {
        ...
        if (session('otp_attempts', 0) >= 3) {
            ...
            return redirect()
                ->route('signup')
                ->withErrors(['otp' => 'Bạn đã nhập sai OTP quá 3 lần, vui lòng đăng ký lại.']);
        }
        ...
        if (now()->greaterThan(session('otp_expires_at'))) {
            ...
            return redirect()
                ->route('signup')
                ->withErrors(['otp' => 'OTP đã hết hạn, vui lòng đăng ký lại.']);
        }
