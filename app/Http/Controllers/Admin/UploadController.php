<?php
    ...
    class UploadController extends Controller
    {
        ...
        public function uploadFile(Request $request)
        {
                ...
                'upload' => [
                    ...
                    function ($attribute, $value, $fail) use (
                        ...
                        if (!in_array($extension,
                            array_merge($allowedImages, $allowedVideos))) {
                            $fail('Chỉ hỗ trợ tải lên ảnh và video.');
                        }
                        if (in_array($extension,
                                $allowedImages) && $file->getSize() > 5 * 1024 * 1024) {
                            $fail('Ảnh không được vượt quá 5MB.');
                        }
                        if (in_array($extension,
                                $allowedVideos) && $file->getSize() > 50 * 1024 * 1024) {
            ...
            $folder = in_array($extension,
                $allowedImages) ? 'ckeditor_images' : 'ckeditor_videos';
