...
class CommentReaction extends Model
{
    ...
    public $timestamps = false; // Tên bảng trong database
    ...
    private function handleCommentReaction($commentId, $isLike)
    {
        ...
        if (! $userId) {
            return response()->json([
                'success' => false,
                'message' => 'Bạn cần đăng nhập để thực hiện thao tác!',
            ]);
        }
        ...
        $likeCount = CommentReaction::where('comment_id', $commentId)
            ->where('is_like', true)
            ->count();
        $dislikeCount = CommentReaction::where('comment_id', $commentId)
            ->where('is_like', false)
            ->count();
