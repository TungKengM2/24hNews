...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        fetch('/admin/tinymce/clear-blocked-images')
    ...
    tinymce
        .init({
                ...
                allElements.forEach(function(el) {
                    if (el.style) {
                        ...
                        if (el.tagName === 'IMG') {
                            ...
                            // Đánh dấu tất cả hình ảnh cần được kiểm duyệt
                            if (el.src && (el.src.startsWith('http') || el.src.startsWith('data:image'))) {
                                ...
                                // Xóa các thuộc tính từ TinyMCE để tránh xung đột
                ...
                if (brs.length > 0) {
                    [...brs].forEach(br => {
                        if (br.parentNode && br.parentNode.tagName !== 'P' &&
                            br.parentNode.tagName !== 'DIV' &&
                            br.parentNode.tagName !== 'LI' &&
                            br.parentNode.tagName !== 'TD' &&
                            br.parentNode.tagName !== 'TH') {
            ...
            setup: function(editor) {
                ...
                function scanAndProcessImages() {
                    ...
                    [...images].forEach(function(img) {
                        ...
                        if (originalSrc.startsWith('data:image')) {
                            fetch(originalSrc)
                                ...
                                .then(result => {
                                    ...
                                    if (result.blocked === true) {
                                        ...
                                        if (result.location) {
                                            console.log('Cập nhật nguồn ảnh data:image thành:', result.location);
                            ...
                            // Đảm bảo tất cả URL đều được kiểm duyệt, không phân biệt domain
                            fetch('/api/moderate/image-url', {
                                ...
                                .then(result => {
                                    ...
                                    console.log('Kết quả kiểm duyệt URL:', originalSrc, result);
                                    
                                    if (result.status === 'success' && result.violation_level ===
                                        'none') {
                                        if (result.location) {
                                            console.log('Cập nhật nguồn ảnh từ:', originalSrc, 'thành:', result.location);
                                            img.setAttribute('src', result.location);
                                            // Đảm bảo ảnh được hiển thị
                                            img.style.display = '';
                                            img.style.visibility = 'visible';
                                        } else {
                                            console.log('Giữ nguồn ảnh gốc vì không có location mới:', originalSrc);
                                        }
                                        img.setAttribute('data-moderated', 'true');
                                        img.setAttribute('data-no-remoderation', 'true');
                                        img.setAttribute('moderated', 'true');
                        ...
                        if (e.clipboardData && e.clipboardData.items) {
                            ...
                            for (let i = 0; i < items.length; i++) {
                                if (items[i].type.indexOf('image/') === 0) {
                                    console.log('Có hình ảnh trong clipboard:', items[i].type);
                                    // Không cần xử lý trực tiếp tại đây, TinyMCE sẽ tự chuyển thành data:image
                                    // và sau đó sẽ được bắt bởi scanAndProcessImages
                                    setTimeout(scanAndProcessImages, 200);
                                    break;
                                }
                            }
                        }
                ...
                // Thêm sự kiện PastePreProcess và PastePostProcess
                editor.on('PastePreProcess', function(e) {
                    ...
                    if (e.content && e.content.indexOf('<img') >= 0) {
                        ...
                        const images = div.querySelectorAll('img');
                        if (images.length > 0) {
                            console.log('Tìm thấy ' + images.length + ' hình ảnh trong PastePreProcess');
                ...
                editor.on('PastePostProcess', function(e) {
                    ...
                    const images = e.node.querySelectorAll('img[data-need-moderation="true"]');
                    if (images.length > 0) {
                        console.log('Xử lý ' + images.length + ' hình ảnh sau khi paste');
                        setTimeout(scanAndProcessImages, 100);
                    }
                });

                // Xử lý sự kiện drop cho kéo thả hình ảnh
                editor.on('drop', function(e) {
                    console.log('Drop event được kích hoạt', e);
                    ...
                    if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                        ...
                        for (let i = 0; i < files.length; i++) {
                            if (files[i].type.indexOf('image/') === 0) {
                                console.log('Có ảnh kéo thả:', files[i].name);
                                // Đánh dấu để xử lý sau khi thả
                                setTimeout(scanAndProcessImages, 100);
                            }
                        }
                    } else {
                        // Xử lý trường hợp kéo thả nội dung HTML (có thể chứa hình ảnh)
                        console.log('Kéo thả nội dung HTML');
                        setTimeout(function() {
                            const newImages = editor.getBody().querySelectorAll('img:not([data-moderated]):not([data-need-moderation])');
                            if (newImages.length > 0) {
                                console.log('Tìm thấy ' + newImages.length + ' hình ảnh mới sau khi kéo thả');
