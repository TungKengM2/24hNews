...
Route::middleware(['auth', 'role:2'])->prefix('author')->group(function () {
    ...
    // Thêm route để xử lý yêu cầu chỉnh sửa
    Route::post('/articles/{article_id}/edit-request', [\App\Http\Controllers\Author\ArticleEditRequestController::class, 'store'])
        ->name('author.articles.edit-request.store');
});
...
Route::post('/notifications/{id}/read', function ($id) {
    ...
    return response()->json(['success' => false], 404); // Trả về 404 nếu không tìm thấy
});
...
Route::middleware(['auth', 'role:1'])->prefix('admin')->group(function () {
    ...
    Route::get('/article-edit-requests', [App\Http\Controllers\Admin\ArticleEditRequestController::class, 'index'])
        ->name('admin.article-edit-requests.index');
    Route::post('/article-edit-requests/{id}/approve', [App\Http\Controllers\Admin\ArticleEditRequestController::class, 'approve'])
        ->name('admin.article-edit-requests.approve');
    Route::post('/article-edit-requests/{id}/reject', [App\Http\Controllers\Admin\ArticleEditRequestController::class, 'reject'])
        ->name('admin.article-edit-requests.reject');
});
...
Route::middleware(['auth'])->group(function () {
    ...
    Route::post('/notifications/clear', [NotificationController::class, 'clearAll'])->name('notifications.clear-all');
    Route::get('/notifications/unread-count', [NotificationController::class, 'countUnread'])->name('notifications.count-unread');
});
